...
export default function TombolSimpanFormTransaksi() {
  ...
  const apakahTombolNonaktif = (() => {
    if (transaksi.jumlah <= 0 || transaksi.dompet_id === 0) {
      return true;
    }
  ...
  const handleSimpan = async () => {
    ...
    try {
      ...
      // Penanganan error sekarang terpusat di context, tidak perlu alert di sini.
      console.error('Gagal menyimpan dari TombolSimpan:', error.message);
    }
  };
